...
const EditPost = () => {
  ...
  const getPostById = async () => {
    try {
      ...
      setUserId(user_Id);
      var imagePaths = response.data.image.split(",").map((path) => path.replace('uploads\\', "/"));
      setImages(imagePaths); // Set the images state with all image paths
      // Rest of your code...
      
    for(var i =0; i<imagePaths.length; i++)
    {
      setImages(imagePaths[i])
      i++
    }
